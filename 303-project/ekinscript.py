...
# --- AYARLAR ---
WEATHER_API_KEY = "94edaaa4dee565fbb85e85544fc11e16"
GEMINI_API_KEY = "AIzaSyAI42r5fk5xVri2ncOwKBrwoGu6Lo2DIcQ"
...
<div class="ticker-wrapper">
    <div class="ticker-text">
        ✨ Bugün ne giyeceğine karar veremiyor musun? AI Stil Asistanı senin için burada! 🧥 Hava durumuna göre en şık kombinler bir tık uzağında. ⚡ Tarzını seç, şehrini belirle ve fark yarat! 🚀
    </div>
</div>
...
/* Kontrol Panelinin Genel Kutusu */
.controls {
    display: flex;
    flex-wrap: wrap;
    ...
    margin-bottom: 40px;
    padding: 25px;
    background: rgba(255, 255, 255, 0.05); /* Çok hafif bir arka plan */
    border-radius: 20px;
}

/* Seçim Kutuları (Dropdowns) */
select {
    padding: 15px 25px; /* İç boşluğu artırarak büyüttük */
    font-size: 16px;
    border-radius: 12px;
    border: 2px solid #6a7cf7; /* Belirgin bir çerçeve */
    background: white;
    color: #333;
    cursor: pointer;
    min-width: 180px;
...
/* Görüntüle ve Favori Butonları */
.main-btn {
    padding: 15px 35px; /* Butonları iyice büyüttük */
    font-size: 18px;
    font-weight: bold;
    text-transform: uppercase; /* Harfleri büyüttük */
    letter-spacing: 1px;
    border-radius: 12px;
    background: linear-gradient(135deg, #4CAF50, #2E7D32); /* Gradyan renk */
    color: white;
    border: none;
    cursor: pointer;
...
.main-btn:hover {
    transform: translateY(-3px); /* Üstüne gelince hafif yükselir */
...
/* Hastayım Checkbox Alanı */
label[style*="display:flex"] {
    font-size: 16px;
    padding: 12px 20px !important;
    border: 2px solid #ff6b6b !important; /* Hastalık için dikkat çekici kırmızı */
    cursor: pointer;
}
/* Alt Başlık Akan Yazı Alanı */
.ticker-wrapper {
    width: 100%;
    overflow: hidden; /* Dışarı taşan yazıları gizle */
    background: rgba(106, 124, 247, 0.1);
    padding: 10px 0;
    margin-bottom: 30px;
...
.ticker-text {
    white-space: nowrap; /* Yazıyı tek satıra zorla */
    display: inline-block;
    font-size: 16px;
    font-weight: 500;
    color: #e0e0e0;
    padding-left: 100%; /* Başlangıç pozisyonu */
    animation: tickerMove 20s linear infinite; /* 20 saniyede bir döngü */
}
...
/* Üzerine gelince yazıyı durdur (Opsiyonel) */
